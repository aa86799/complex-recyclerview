 ...
 *
 * BaseAdapter<M, VH extends BaseHolder> extends AbsAdapter<VH>
 *     > 对数据的操作： 添加、删除
 */
...
public abstract class BaseAdapter<M, VH extends BaseHolder> extends AbsAdapter<VH> {
     ...
     * @param data 要前置的数据
     */
    public void appendFirstItem(M data) {
        dataList.add(0, data);
        if (getHeaderExtraViewCount() == 0) {
     ...
     * @param list 要前置的数据集合
     */
    public void appendFirstList(List<M> list) {
    ...
    /**
     * 移除多条数据 [startIndex, endIndex)
     * @param startIndex 数据开始索引
     * @param endIndex 数据结束索引
     */
    public void removeItem(int startIndex, int endIndex) {
        int lastIndex = dataList.size() - 1;
        if (lastIndex == -1 || startIndex <= -1 || endIndex < startIndex) return;

        dataList.remove(dataList.subList(startIndex, endIndex));
